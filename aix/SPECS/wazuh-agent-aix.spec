...
Summary:     The Exact-Ti agent, used for threat detection, incident response and integrity monitoring.
...
%description
Exact-Ti is an open source security monitoring solution for threat detection, integrity monitoring, incident response and compliance.

%prep
%setup -q
deps_version=`cat src/Makefile | grep "DEPS_VERSION =" | cut -d " " -f 3`
cd src && gmake clean && gmake deps RESOURCES_URL=http://packages.exactti.com/deps/${deps_version} TARGET=agent
...
/opt/freeware/bin/install -m 0750 src/init/templates/ossec-hids-aix.init ${RPM_BUILD_ROOT}%{_init_scripts}/exactti-agent
...
if [ $1 = 2 ]; then
  if /etc/rc.d/init.d/exactti-agent status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    /etc/rc.d/init.d/exactti-agent stop > /dev/null 2>&1 || :
...
if [ $1 = 1 ]; then
  ...
  # Restore Exact-Ti agent configuration
...
ln -fs /etc/rc.d/init.d/exactti-agent /etc/rc.d/rc2.d/S97exactti-agent
ln -fs /etc/rc.d/init.d/exactti-agent /etc/rc.d/rc3.d/S97exactti-agent
...
if [ -f %{_localstatedir}/tmp/wazuh.restart ]; then
  ...
  /etc/rc.d/init.d/exactti-agent restart > /dev/null 2>&1 || :
fi
