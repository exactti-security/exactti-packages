...
%global DASHBOARD_FILE exactti-dashboard-base-%{version}-%{release}-linux-x64.tar.xz
...
cp exactti-dashboard-base/etc/node.options %{buildroot}%{CONFIG_DIR}
cp exactti-dashboard-base/etc/opensearch_dashboards.yml %{buildroot}%{CONFIG_DIR}
cp exactti-dashboard-base/VERSION %{buildroot}%{INSTALL_DIR}
...
cp %{buildroot}%{INSTALL_DIR}/etc/services/exactti-dashboard.service %{buildroot}/etc/systemd/system/exactti-dashboard.service
cp %{buildroot}%{INSTALL_DIR}/etc/services/default %{buildroot}/etc/default/exactti-dashboard

chmod 640 %{buildroot}/etc/systemd/system/exactti-dashboard.service
...
chown root:root %{buildroot}/etc/systemd/system/exactti-dashboard.service

if [ "%{version}" = "99.99.0" ];then
    runuser %{USER} --shell="/bin/bash" --command="%{buildroot}%{INSTALL_DIR}/bin/opensearch-dashboards-plugin install https://packages-dev.exactti.com/futures/ui/dashboard/wazuh-99.99.0-%{release}.zip"
    runuser %{USER} --shell="/bin/bash" --command="%{buildroot}%{INSTALL_DIR}/bin/opensearch-dashboards-plugin install https://packages-dev.exactti.com/futures/ui/dashboard/wazuhCheckUpdates-99.99.0-%{release}.zip"
    runuser %{USER} --shell="/bin/bash" --command="%{buildroot}%{INSTALL_DIR}/bin/opensearch-dashboards-plugin install https://packages-dev.exactti.com/futures/ui/dashboard/wazuhCore-99.99.0-%{release}.zip"
else
...
%attr(750, %{USER}, %{GROUP}) "/etc/default/exactti-dashboard"
%attr(640, %{USER}, %{GROUP}) "%{CONFIG_DIR}/node.options"
%attr(640, root, root) "/etc/systemd/system/exactti-dashboard.service"
