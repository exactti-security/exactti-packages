...
# Foundation.
Summary:     Exact-Ti indexer is a search and analytics engine for security-related data. Documentation can be found at https://documentation.exactti.com/current/getting-started/components/exactti-indexer.html
Name:        exactti-indexer
Version:     %{_version}
Release:     %{_release}
License:     GPL
...
%global INDEXER_FILE exactti-indexer-base-%{version}-%{release}-linux-x64.tar.xz

# -----------------------------------------------------------------------------

%description
Exact-Ti indexer is a near real-time full-text search and analytics engine that gathers security-related data into one platform. This Exact-Ti central component indexes and stores alerts generated by the Exact-Ti server. Exact-Ti indexer can be configured as a single-node or multi-node cluster, providing scalability and high availability. Documentation can be found at https://documentation.exactti.com/current/getting-started/components/exactti-indexer.html
...
chown -R %{USER}:%{GROUP} exactti-indexer-*/*

# Copy base files into RPM_BUILD_ROOT directory
mv exactti-indexer-*/etc ${RPM_BUILD_ROOT}/
mv exactti-indexer-*%{SYS_DIR}/* ${RPM_BUILD_ROOT}%{SYS_DIR}/
rm -rf exactti-indexer-*/etc
rm -rf exactti-indexer-*/usr
cp -pr exactti-indexer-*/* ${RPM_BUILD_ROOT}%{INSTALL_DIR}/
...
# Copy Exact-Ti's config files for the security plugin
...
chmod 750 ${RPM_BUILD_ROOT}/etc/init.d/exactti-indexer
...
if [ $1 = 2 ]; then
  ...
  # Check for SysV
    ...
    touch %{INSTALL_DIR}/%{name}.restart
      ...
      if command -v invoke-rc.d >/dev/null && invoke-rc.d --quiet exactti-indexer status > /dev/null 2>&1; then
...
if [ $1 = 1 ];then # Install
    ...
    # To pick up /usr/lib/sysctl.d/exactti-indexer.conf
...
if [ $1 = 0 ];then # Remove
    echo -n "Stopping exactti-indexer service..."
...
if [ -f %{INSTALL_DIR}/%{name}.restart ]; then
    echo -n "Starting exactti-indexer service..."
