...
function passwords_changePassword() {
    ...
    if [ -n "${changeall}" ]; then
        if [ -n "${indexer_installed}" ] && [ -z ${no_indexer_backup} ]; then
            eval "mkdir /etc/exactti-indexer/backup/ ${debug}"
            eval "cp /etc/exactti-indexer/opensearch-security/* /etc/exactti-indexer/backup/ ${debug}"
            passwords_createBackUp
        fi

        for i in "${!passwords[@]}"
        do
            if [ -n "${indexer_installed}" ] && [ -f "/etc/exactti-indexer/backup/internal_users.yml" ]; then
                awk -v new=${hashes[i]} 'prev=="'${users[i]}':"{sub(/\042.*/,""); $0=$0 new} {prev=$1} 1' /etc/exactti-indexer/backup/internal_users.yml > internal_users.yml_tmp && mv -f internal_users.yml_tmp /etc/exactti-indexer/backup/internal_users.yml
            fi
        ...
        if [ -n "${indexer_installed}" ] && [ -f "/etc/exactti-indexer/backup/internal_users.yml" ]; then
            awk -v new='"'"${hash}"'"' 'prev=="'${nuser}':"{sub(/\042.*/,""); $0=$0 new} {prev=$1} 1' /etc/exactti-indexer/backup/internal_users.yml > internal_users.yml_tmp && mv -f internal_users.yml_tmp /etc/exactti-indexer/backup/internal_users.yml
        fi
    ...
    if [ "${nuser}" == "admin" ] || [ -n "${changeall}" ]; then
        if [ -n "${filebeat_installed}" ]; then
            ...
            passwords_restartService "exactti-server"
        fi
    fi

    if [ "$nuser" == "kibanaserver" ] || [ -n "$changeall" ]; then
        if [ -n "${dashboard_installed}" ] && [ -n "${dashpass}" ]; then
            if /usr/share/exactti-dashboard/bin/opensearch-dashboards-keystore --allow-root list | grep -q opensearch.password; then
                eval "echo ${dashpass} | /usr/share/exactti-dashboard/bin/opensearch-dashboards-keystore --allow-root add -f --stdin opensearch.password ${debug_pass} > /dev/null 2>&1"
            else
                wazuhdashold=$(grep "password:" /etc/exactti-dashboard/opensearch_dashboards.yml )
                rk="opensearch.password: "
                wazuhdashold="${wazuhdashold//$rk}"
                conf="$(awk '{sub("opensearch.password: .*", "opensearch.password: '"${dashpass}"'")}1' /etc/exactti-dashboard/opensearch_dashboards.yml)"
                echo "${conf}" > /etc/exactti-dashboard/opensearch_dashboards.yml
            fi
            passwords_restartService "exactti-dashboard"
...
function passwords_changePasswordApi() {
    ...
    if [ -f "/usr/share/exactti-dashboard/data/wazuh/config/wazuh.yml" ]; then
        wazuh_yml_user=$(awk '/- default:/ {found=1} found && /username:/ {print $2}' /usr/share/exactti-dashboard/data/wazuh/config/wazuh.yml)
    fi
    if [ -n "${changeall}" ]; then
        for i in "${!api_passwords[@]}"; do
            if [ -n "${wazuh_installed}" ]; then
                ...
                if [ -z "${AIO}" ] && [ -z "${indexer}" ] && [ -z "${dashboard}" ] && [ -z "${wazuh}" ] && [ -z "${start_indexer_cluster}" ]; then
                    common_logger -nl $"The password for Exact-Ti API user ${api_users[i]} is ${api_passwords[i]}"
                fi
            fi
        ...
        if [ -n "${wazuh_installed}" ]; then
            ...
            eval 'common_curl -s -k -X PUT -H \"Authorization: Bearer $TOKEN_API\" -H \"Content-Type: application/json\" -d "$WAZUH_PASS_API" "https://localhost:55000/security/users/${user_id}" -o /dev/null --max-time 300 --retry 5 --retry-delay 5 --fail'
            if [ -z "${AIO}" ] && [ -z "${indexer}" ] && [ -z "${dashboard}" ] && [ -z "${wazuh}" ] && [ -z "${start_indexer_cluster}" ]; then
                common_logger -nl $"The password for Exact-Ti API user ${nuser} is ${password}"
            fi
        fi
...
function passwords_changeDashboardApiPassword() {
    ...
    until [ -n "${file_exists}" ] || [ "${j}" -eq "12" ]; do
        if [ -f "/usr/share/exactti-dashboard/data/wazuh/config/wazuh.yml" ]; then
            eval "sed -i 's|password: .*|password: \"${1}\"|g' /usr/share/exactti-dashboard/data/wazuh/config/wazuh.yml ${debug}"
...
function passwords_createBackUp() {
    ...
    if [ -z "${indexer_installed}" ] && [ -z "${dashboard_installed}" ] && [ -z "${filebeat_installed}" ]; then
        common_logger -e "Cannot find Exact-Ti indexer, Exact-Ti dashboard or Filebeat on the system."
        exit 1;
    else
        if [ -n "${indexer_installed}" ]; then
            capem=$(grep "plugins.security.ssl.transport.pemtrustedcas_filepath: " /etc/exactti-indexer/opensearch.yml )
    ...
    if [ ! -d "/etc/exactti-indexer/backup" ]; then
        eval "mkdir /etc/exactti-indexer/backup ${debug}"
    fi
    eval "JAVA_HOME=/usr/share/exactti-indexer/jdk/ OPENSEARCH_CONF_DIR=/etc/exactti-indexer /usr/share/exactti-indexer/plugins/opensearch-security/tools/securityadmin.sh -backup /etc/exactti-indexer/backup -icl -p 9200 -nhnv -cacert ${capem} -cert ${adminpem} -key ${adminkey} -h ${IP} ${debug}"
    ...
    common_logger -d "Password backup created in /etc/exactti-indexer/backup."
...
function passwords_generateHash() {
    ...
    if [ -n "${changeall}" ]; then
        ...
        for i in "${!passwords[@]}"
        do
            nhash=$(bash /usr/share/exactti-indexer/plugins/opensearch-security/tools/hash.sh -p "${passwords[i]}" 2>&1 | grep -A 2 'issues' | tail -n 1)
            if [  "${PIPESTATUS[0]}" != 0  ]; then
        ...
        hash=$(bash /usr/share/exactti-indexer/plugins/opensearch-security/tools/hash.sh -p "${password}" 2>&1 | grep -A 2 'issues' | tail -n 1)
        if [  "${PIPESTATUS[0]}" != 0  ]; then
            common_logger -e "Hash generation failed."
            if [[ $(type -t installCommon_rollBack) == "function" ]]; then
                installCommon_rollBack
...
function passwords_generatePasswordFile() {
    ...
    user_description=(
        "Admin user for the web user interface and Exact-Ti indexer. Use this user to log in to Exact-Ti dashboard"
        "Exact-Ti dashboard user for establishing the connection with Exact-Ti indexer"
        "Regular Dashboard user, only has read permissions to all indices and all permissions on the .kibana index"
        "Filebeat user for CRUD operations on Exact-Ti indices"
        "User with READ access to all indices"
        "User with permissions to perform snapshot and restore operations"
        "Admin user used to communicate with Exact-Ti API"
        "Regular user to query Exact-Ti API"
    )
...
function passwords_getApiToken() {
    ...
    while [[ "${TOKEN_API}" =~ "Exact-Ti Internal Error" ]] && [ "${retries}" -lt "${max_internal_error_retries}" ]
    do
    ...
    if [[ ${TOKEN_API} =~ "Exact-Ti Internal Error" ]]; then
...
function passwords_getApiUserId() {
    ...
    if [ "${user_id}" == "noid" ]; then
        common_logger -e "User ${1} is not registered in Exact-Ti API"
        if [[ $(type -t installCommon_rollBack) == "function" ]]; then
                installCommon_rollBack
        fi
        exit 1
...
function passwords_getNetworkHost() {
    ...
    IP=$(grep -hr "^network.host:" /etc/exactti-indexer/opensearch.yml)
    NH="network.host: "
    IP="${IP//$NH}"
...
function passwords_readUsers() {
    ...
    if [ -n "${indexer_installed}" ]; then
        ...
        susers=$(grep -B 1 hash: /etc/exactti-indexer/opensearch-security/internal_users.yml | grep -v hash: | grep -v "-" | awk '{ print substr( $0, 1, length($0)-1 ) }')
...
function passwords_runSecurityAdmin() {
    ...
    eval "OPENSEARCH_CONF_DIR=/etc/exactti-indexer /usr/share/exactti-indexer/plugins/opensearch-security/tools/securityadmin.sh -f /etc/exactti-indexer/backup/internal_users.yml -t internalusers -p 9200 -nhnv -cacert ${capem} -cert ${adminpem} -key ${adminkey} -icl -h ${IP} ${debug}"
    if [  "${PIPESTATUS[0]}" != 0  ]; then
        common_logger -e "Could not load the changes."
        exit 1;
    fi
    eval "cp /etc/exactti-indexer/backup/internal_users.yml /etc/exactti-indexer/opensearch-security/internal_users.yml"
    eval "rm -rf /etc/exactti-indexer/backup/ ${debug}"

    if [[ -n "${nuser}" ]] && [[ -n ${autopass} ]]; then
        common_logger -nl "The password for user ${nuser} is ${password}"
        common_logger -w "Password changed. Remember to update the password in the Exact-Ti dashboard, Exact-Ti server, and Filebeat nodes if necessary, and restart the services."
    fi
    ...
    if [ -n "${changeall}" ]; then
        if [ -z "${AIO}" ] && [ -z "${indexer}" ] && [ -z "${dashboard}" ] && [ -z "${wazuh}" ] && [ -z "${start_indexer_cluster}" ]; then
            ...
            common_logger -w "Exact-Ti indexer passwords changed. Remember to update the password in the Exact-Ti dashboard, Exact-Ti server, and Filebeat nodes if necessary, and restart the services."
        else
...
function passwords_updateInternalUsers() {
    ...
    internal_users_backup_path="/etc/exactti-indexer/internalusers-backup"
    passwords_getNetworkHost
    passwords_createBackUp

    eval "mkdir -p ${internal_users_backup_path} ${debug}"
    eval "cp /etc/exactti-indexer/backup/internal_users.yml ${internal_users_backup_path}/internal_users_${backup_datetime}.yml.bkp ${debug}"
    ...
    eval "chown -R exactti-indexer:exactti-indexer ${internal_users_backup_path} ${debug}"
    common_logger "A backup of the internal users has been saved in the /etc/exactti-indexer/internalusers-backup folder."

    eval "cp /etc/exactti-indexer/backup/internal_users.yml /etc/exactti-indexer/opensearch-security/internal_users.yml ${debug}"
    eval "rm -rf /etc/exactti-indexer/backup/ ${debug}"
